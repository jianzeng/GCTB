...
//        cout << i << " " << sizei << endl;
        ...
        my_rhsMale.setZero(sizei);
        my_rhsFemaleNDC.setZero(sizei);
        
        for (unsigned j=0; j<sizei; ++j) {
            snpj = windowSnpIdxVec[i][j];
            oldSampleMale[j] = values(snpj,0);
            oldSampleFemale[j] = values(snpj,1);
            my_rhsMale[j] = Z.col(snpj).head(nmale).dot(ycorrm);
            my_rhsFemaleNDC[j] = Z.col(snpj).tail(nfemale).dot(ycorrf);
        }
...
//        cout << "i " << i << endl;
        ...
        MatrixXf my_Zmale(nmale,sizei);
        MatrixXf my_Zfemale(nfemale,sizei);
        MatrixXf my_ZPZblockDiagMale(sizei,sizei);
        MatrixXf my_ZPZblockDiagFemale(sizei,sizei);
        for (unsigned j=0; j<sizei; ++j) {
//            cout << "j " << j << endl;
            unsigned snpj = data.windowSnpIdxVec[i][j];
            my_Zmale.col(j) = data.Z.col(snpj).head(nmale);
            my_Zfemale.col(j) = data.Z.col(snpj).tail(nfemale);
        }
        my_ZPZblockDiagMale = my_Zmale.transpose() * my_Zmale;
        my_ZPZblockDiagFemale = my_Zfemale.transpose() * my_Zfemale;
        //MPI
        MPI_Allreduce(my_ZPZblockDiagMale.data(), ZPZblockDiagMale[i].data(), sizei*sizei, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
        MPI_Allreduce(my_ZPZblockDiagFemale.data(), ZPZblockDiagFemale[i].data(), sizei*sizei, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
    }
}
